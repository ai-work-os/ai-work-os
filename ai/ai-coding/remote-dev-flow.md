...
不论从哪触发(Mac 直接 / 手机 → 频道 dispatcher),都走 [start-task skill](skills/start-task/SKILL.md),产出同一种 Workspace。对人和流程层统一叫 Workspace;底层实现是 git worktree,工具名仍是 `worktree-task`。
...
├── AGENTS.md / CLAUDE.md    ← 根仓库 Workspace 顶层
├── ai/                       ← 项目知识
├── TASK.md                   ← 任务卡(AGENTS.md 顶部引导自动读)
├── nerve/                    ← 三个代码仓 Workspace(分支 <type>/<id>,底层 git worktree)
├── nerve-app/
└── nerve-tui/
```
...
3. `worktree-task create` 从配置 remote/base 最新 commit 建 Workspace。
4. worker 在任务 Workspace 内改代码、跑测试、提交分支。
...
- TASK.md / plan.md / progress.md 是任务工作区临时文件,留在 Workspace 顶层,**不进 git**。
