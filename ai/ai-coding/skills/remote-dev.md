...
5. **建隔离区**(在 shell 里跑;路径已正确):
   ...
   ```
   不传 `--repos`(默认全切根 + 三代码仓)。worktree 结构跟主仓库一模一样:根仓库 worktree 占顶层,子代码仓嵌套。
...
7. **spawn worker** —— 用 `nerve_spawn` 工具:`adapter`=`claude`、`name`=`worker-<id>`、
   ...
   给 `worker-<id>` 发起始指令:
   > 你是 worker。先读 `../TASK.md`(任务卡)和 `../AGENTS.md`(根仓库 worktree 顶层项目入口),
...
1. **装上下文** —— 读 `../TASK.md`(任务卡)+ `../AGENTS.md`(根仓库 worktree 顶层)+ `CLAUDE.md`(铁律)。
