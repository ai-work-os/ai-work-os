...
**职责**:开发需求来了,走 `start-task` skill 三件事 —— 澄清需求、按远端 baseline 预侦察代码、填好 Workspace 顶层的 `TASK.md`。任务做完、要发 MR、要发版或要清理时,走 `finish-task` skill 收尾;合入或授权直推完成后默认运行 `finish-task complete`,不要等用户提醒清 Workspace。

流程分 A+B 档:
- A 档(日常开发):GitLab Issue 入口 → 远端 baseline Workspace → worker 实现测试 → GitLab MR 关联 Issue → main 保护分支只经 MR 合并。
